  ...
  function attachOrderHandlers() {
    document.querySelectorAll(".order-button").forEach(function (button) {
      button.addEventListener("click", function (e) {
        ...
        var pizzaImageUrl = rootElement.querySelector('.pizza-image').getAttribute('src')
        var pizzaPrice = +rootElement.querySelector('.price-accent').textContent.slice(0, -2);
        ...
        if (pizzaSize == null) {
          ...
          idbKeyval.get("order").then(function (order) {
            ...
            idbKeyval.set('order', order).then(function () {
              iziToast.success({
                id: 'success',
